...
export default function Historial() {
  ...
  // Post-corte Fase 3: todo se edita en /pedido (el redirect cubre pedidos
  // viejos que hayan quedado con links a /presupuesto).
  const pedidoRoute = (b) => `/pedido/${b.id}`
  const editB = (id) => nav(`/pedido/${id}`)

  // ── Duplicar pedido → nuevo pedido con los datos precargados ──
                    ...
                    <tr key={b.id} className={selectedIds.has(b.id) ? 'selected' : ''} style={selectedIds.has(b.id) ? { background: 'var(--brand-xlt)' } : undefined}>
                      ...
                      <td data-cell="pago" style={{ whiteSpace: 'nowrap', verticalAlign: 'middle' }}>
                        {/* Celda PAGO minimalista (07/07):
                            ...
                            Fila 2: CTA "Cobrar" (pending) o resumen chico (partial/paid)
                            Sin barra, sin %, sin desglose — el detalle vive en el modal Pagos. */}
                        {(() => {
                          const totalDue    = b.totalFinal || b.total || 0
                          const paid        = cobrado(b)
                          ...
                          return (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, minWidth: 130, maxWidth: 170 }}>
                              {/* Fila 1: chip estado (dropdown editable) */}
                              ...
                              {/* Fila 2: acción o resumen mínimo */}
